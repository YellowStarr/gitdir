...
class HandleExcel:
    ...
    def read_testcase(self,sheetname):    #获取每行测试用例,返回用例数组
        ...
        for row in range(1, rows):
            ...
            case['host'] = table.cell_value(row, 1)
            case['url'] = table.cell_value(row, 2)
            case['method'] = table.cell_value(row, 3)
            case['args'] = table.cell_value(row, 4)
            case['expcode'] = table.cell_value(row, 5)
            case['expdata'] = table.cell_value(row, 6)
    ...
    def write_result(self, workbook, sheetname, result, resultfile):    #将测试结果写入excel，传入测试用例文件，测试结果数组，及保存文件名
        ...
        if not isinstance(result,list):
            ...
            for i in range(0,len(result)):
                ...
                table.write(int(row), 7, result[i]['rcode'])
                table.write(int(row), 8, str(result[i]['rdata']))
                if result[i]['result'] == 'fail':    #如果测试结果为fail 则该单元格标红
                    ...
                    table.write(int(row), 9, 'fail', style)
